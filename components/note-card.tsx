...
export default function NoteCard({ note, onClick, onDelete }: NoteCardProps) {
  ...
  return (
    <Card
      className="h-full min-h-[220px] cursor-pointer overflow-hidden border-t-4 hover:shadow-md transition-all duration-200 active:scale-[0.98] relative"
      style={{ borderTopColor: note.color || "#6366f1" }}
      onClick={onClick}
    >
      <CardContent className="p-4 sm:p-5 pb-12">
      ...
      <Button
        ...
        className="absolute bottom-2 right-2 h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10"
        title="Delete note"
      >
